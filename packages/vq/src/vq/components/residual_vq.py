...
class ResidualVQ(nn.Module):
    ...
    def __init__(
        ...
        if shared_codebook:
            layer = QuantizeEMAReset(nb_code=nb_code, code_dim=code_dim, mu=mu)
            self.layers = nn.ModuleList([layer for _ in range(num_quantizers)])
        else:
            self.layers = nn.ModuleList([QuantizeEMAReset(nb_code=nb_code, code_dim=code_dim, mu=mu) for _ in range(num_quantizers)])
