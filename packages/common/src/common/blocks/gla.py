...
from .attentive_rnn import AttentiveRNN
from .crossatt import CrossAttention, BlindCrossAttention, CrossAttentionPP
from .base_blocks import MixingBlock, SwiGLU
...
class GatedLinearAttention(nn.Module):
	...
	def __init__(
		...
		if use_short_conv:
			...
			if share_conv_kernel:
				...
				self.q_conv1d = ShortConvolution(self.key_dim, conv_size, activation='silu', bias=True)
				self.k_conv1d = ShortConvolution(self.key_dim, conv_size, activation='silu', bias=True)
				self.v_conv1d = ShortConvolution(self.value_dim, conv_size, activation='silu', bias=True)
	...
	@torch.no_grad()
	def enable_fused_qkv(self):
		"""
		Create a fused Linear for [Q;K;V] and copy existing weights.
		Safe for inference; no retrain needed.
		"""
		if not hasattr(self, 'qkvg'):
			self.qkvg_fused = False     
			self.qkvg = None

		if self.qkvg is not None:
			self.qkvg_fused = True
			return

		out_dim = self.key_dim * 2 + self.value_dim
		self.qkvg = nn.Linear(self.hidden_size, out_dim, bias=False).to(self.q_proj.weight.device, self.q_proj.weight.dtype)

		# Copy weights: [Q | K | V]
		self.qkvg.weight.data[:self.key_dim].copy_(self.q_proj.weight.data)
		self.qkvg.weight.data[self.key_dim:2*self.key_dim].copy_(self.k_proj.weight.data)
		self.qkvg.weight.data[2*self.key_dim:].copy_(self.v_proj.weight.data)

		# Keep originals intact (for training), but use fused path at inference
		self.qkvg_fused = True
	...
	def forward(
		...
		if self.use_short_conv:
			...
			if self.share_conv_kernel:
				...
				if self.qkvg_fused and self.qkvg is not None:
					qkv = self.qkvg(hidden_states)
				...
				# three convs are costly; consider disabling at inference
				conv_state_q = last_state[0] if use_cache else None
				conv_state_k = last_state[1] if use_cache else None
				conv_state_v = last_state[2] if use_cache else None
				q = self.q_proj(hidden_states); q = self.q_conv1d(q, attention_mask, conv_state_q)
				k = self.k_proj(hidden_states); k = self.k_conv1d(k, attention_mask, conv_state_k)
				v = self.v_proj(hidden_states); v = self.v_conv1d(v, attention_mask, conv_state_v)
		else:
			# --- Fused QKV fast path ---
			if self.qkvg_fused and self.qkvg is not None:
				qkv = self.qkvg(hidden_states)
...
class AttentiveGLA(AttentiveRNN):
	def __init__(
		...
		if cross_att:
			...
			self.cross_att = None
	def forward(self, x, ctx, mask=None, pos=None, reset_mask=None, attention_only=None, forced_attention=None, init_state=None):
	...
	def get_state_from_params(self, params, batch_size, scale=1.):
	...
	def get_init_state_tuning_params(self, lora: Optional[int]=None, scale: float=1., device=None):
		parameters = []
	...
	def step(self, y_embd, x_enc, time_step, cache, kv_cache=None, ca_int=1):
		for e in self.encoder:
			y_embd = e(y_embd, past_key_values=cache, use_cache=True)
		if self.cross_att is not None and (time_step%ca_int)==0:
			v, att = self.cross_att(y_embd, x_enc, time_step=time_step, past_key_values=cache, use_cache=True, kv_cache=kv_cache)
			y_embd = y_embd + v
		else:
			att = None
		for d in self.decoder:
